  ...
  create: function create(context) {
    return {
      JSXOpeningElement: function JSXOpeningElement(node) {
        ...
        const values = propsToValidate.map(prop => (0, _jsxAstUtils.getProp)(node.attributes, prop)).map(prop => (0, _jsxAstUtils.getPropValue)(prop));
        // Checks if any actual or custom href prop is provided.
        const hasAnyHref = values.filter(value => value === undefined || value === null).length !== values.length;
        // Need to check for spread operator as props can be spread onto the element
        // leading to an incorrect validation error.
        const hasSpreadOperator = attributes.filter(prop => prop.type === 'JSXSpreadAttribute').length > 0;
        const onClick = (0, _jsxAstUtils.getProp)(attributes, 'onClick');
        ...
        const invalidHrefValues = values.filter(value => value !== undefined && value !== null).filter(value => typeof value === 'string' && (!value.length || value === '#' || /^\W*?javascript/.test(value)));
