...
const nonInteractiveAXObjects = new Set([].concat(_toConsumableArray(_axobjectQuery.AXObjects.keys())).filter(name => (0, _arrayIncludes2.default)(['window', 'structure'], _axobjectQuery.AXObjects.get(name).type)));

const nonInteractiveElementAXObjectSchemas = [].concat(_toConsumableArray(_axobjectQuery.elementAXObjects)).reduce((accumulator, _ref5) => {
  let _ref6 = _slicedToArray(_ref5, 2),
    elementSchema = _ref6[0],
    AXObjectSet = _ref6[1];

  if ([].concat(_toConsumableArray(AXObjectSet)).every(role => nonInteractiveAXObjects.has(role))) {
    accumulator.push(elementSchema);
  }
  return accumulator;
}, []);
 ...
 */
const isNonInteractiveElement = function isNonInteractiveElement(tagName, attributes) {
