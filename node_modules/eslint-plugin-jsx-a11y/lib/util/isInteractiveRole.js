...
const roles = [].concat(_toConsumableArray(_ariaQuery.roles.keys()));
const interactiveRoles = roles.filter(name => !_ariaQuery.roles.get(name).abstract).filter(name => _ariaQuery.roles.get(name).superClass.some(klasses => (0, _arrayIncludes2.default)(klasses, 'widget')));
 ...
 */
const isInteractiveRole = function isInteractiveRole(tagName, attributes) {
  const value = (0, _jsxAstUtils.getLiteralPropValue)((0, _jsxAstUtils.getProp)(attributes, 'role'));
